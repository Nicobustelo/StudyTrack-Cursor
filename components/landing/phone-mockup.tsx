...
function NodeShell({
  ...
  return (
    <div
      ...
      style={{ left: `${(x / TRACK_W) * 100}%`, top: y }}
    >
      {children}
    </div>
  );
...
export function PhoneMockup({ className }: { className?: string }) {
  return (
    <div className={cn("relative mx-auto w-full max-w-[290px]", className)}>
      {/* Chip flotante: countdown */}
      <div className="absolute left-2 top-4 z-30 flex items-center gap-1.5 rounded-full bg-ink px-3 py-1.5 text-[11px] font-bold text-white shadow-soft motion-safe:animate-float sm:-left-5">
        <Timer className="size-3.5 text-accent-orange" />
        Faltan 8 días
      </div>

      {/* Tarjeta flotante: preparación */}
      <div className="absolute right-2 top-40 z-30 w-40 rounded-2xl bg-surface p-3 shadow-soft ring-1 ring-border motion-safe:animate-float-delayed sm:-right-7">
      ...
      <div className="absolute left-2 bottom-20 z-30 w-48 rounded-2xl bg-surface p-3 shadow-soft ring-1 ring-border motion-safe:animate-float sm:-left-7">
      ...
      <div className="relative overflow-hidden rounded-[44px] border-10 border-ink bg-background shadow-2xl">
        ...
        <div className="flex h-[566px] flex-col pt-10">
          ...
          {/* Camino vertical */}
          <div className="relative mx-auto mt-1 w-full" style={{ height: TRACK_H }}>
            <svg
